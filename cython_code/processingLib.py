...
function return wavelet coefficients for morlet transform 
    """
    # N = signal.size
    fr = np.arange(1, 101)         # np.linspace(1, 200, 400); # vector of frequencies
    w0 = 5    
...
def butter_bandpass_filter(data, lowcut, highcut, fs, order=2):
    ...
    return y    
####################################################################
def getMatFiles (mainDirectory):
    filesList =[]
    directories = os.listdir(mainDirectory)
    
    for dirTmp in directories:
        
        files = os.listdir(mainDirectory + dirTmp + '/')
        for fileTmp in files:
            
            if os.path.splitext(fileTmp)[1] != '.mat':
                continue
            filesList.append(mainDirectory + dirTmp + '/' + fileTmp)
...
def cossfrequency_phase_amp_coupling (signal, fd, ampFrs, phFrs, phasebin):
    ...
    coupling = np.zeros( [np.ceil(2*np.pi/phasebin)+1, ampFrs.size] )
...
def cossfrequency_phase_phase_coupling (signal, fd, phFrs1, phFrs2, nmarray):
    ...
    plt.hist2d(angles1, angles2, bins=(60, 60), norm=LogNorm())     # H, xedges, yedges, img =
    ...
    for i in range(nmarray.shape[1]):
        ...
        # ax = plt.subplot(111, polar=True)
        ax.set_title( str(nmarray[0, i]) + ' * theta : ' + str (nmarray[1, i]) + ' * gama' )
